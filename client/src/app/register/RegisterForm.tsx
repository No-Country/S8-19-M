...
const error = { userName: '', password: '', pwdConfirm: '' }
...
export default function RegisterForm() {
  async function handleSubmit(values: FormData) {
    ...
    const pwdConfirm = values.get('pwdConfirm') as string
    error.userName = ''
    error.password = ''
    error.pwdConfirm = ''
    if (userName === '') {
  ...
  return (
    <article className='relative mx-auto flex h-full max-w-md flex-col justify-center'>
      ...
      <form action={handleSubmit} className='text-center'>
        ...
        <CustomInput error={error.pwdConfirm} label='Confirmar Contraseña' name='pwdConfirm' placeholder='Confirma tu contraseña' type='password' />
